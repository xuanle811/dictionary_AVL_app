...
const AppSang = () => {
  ...
  const [suggestionThreshold, setSuggestionThreshold] = useState(2); // tham số threshold cho fuzzy_full
  ...
  const loadVocabulary = useCallback(async () => {
    try {
      ...
      const normalized = (res.data || []).map((item, idx) => ({ id: item.id ?? idx, word: item.word, definition: item.definition || item.definition || "", example: item.example || "", source: item.source || "backend", createdAt: item.createdAt || item.created_at || null }));
      setWords(normalized);
    } catch (err) {
      console.error(err);
...
}, [searchTerm]); // bạn có thể thêm suggestionThreshold nếu dùng state đó
  ...
  // số ký tự tối thiểu để gọi API suggestions
const MIN_SUGGEST_CHARS = 2;

// debounce ms
const SUGGEST_DEBOUNCE_MS = 300;

// nếu muốn threshold động theo độ dài query
const computeThreshold = (q) => {
  if (!q) return 1;
  const L = q.length;
  if (L <= 3) return 1;
  if (L <= 6) return 2;
  return Math.max(2, Math.floor(L * 0.25));
};
  ...
  try {
    // compute threshold (có thể dùng suggestionThreshold state hoặc dynamic)
    const threshold = computeThreshold(qTrim); // hoặc suggestionThreshold

    // gọi API backend fuzzy
    const res = await apiCall(`/fuzzy_full?query=${encodeURIComponent(qTrim)}&threshold=${encodeURIComponent(threshold)}`);

    // nếu backend không trả về results array -> clear suggestions (không fallback local)
    if (!res || !Array.isArray(res.results)) {
      setSuggestions([]);
      return;
    }

    // normalize kết quả về dạng frontend dùng
    const normalized = res.results.map((r, i) => ({
      id: r.offset ?? i,
      word: r.term ?? r.word ?? "",
      meaning: r.definition ?? r.meaning ?? "",
      definition: r.definition ?? r.meaning ?? "",
      context: Array.isArray(r.context) ? r.context : [],
      metadata: r.metadata || {},
      source: "backend"
    }));

    setSuggestions(normalized.slice(0, 10)); // tối đa 10 gợi ý

  } catch (err) {
    console.error("Fuzzy API failed:", err);
    // không fallback local — chỉ clear suggestions
    setSuggestions([]);
  }
};

  const handleSelectSuggestion = (item) => {
    // đặt từ vào ô tìm kiếm
    setSearchTerm(item.word || "");
    setShowSuggestions(false);

    // show as search result (1 item)
    setSearchResults([{
      id: item.id,
      word: item.word,
    ...
    // Optionally load full detail from backend term_info (offset) if you want:
  ...
  // const handleFileChange = (e) => { const f = e.target.files && e.target.files[0]; if (!f) return; setSelectedFiles(f); setFileName(f.name); };
  ...
  const openDetail = async (wordObj) => {
    try {
      ...
      console.log("Fetching offset:", wordObj.id);

      const res = await apiCall(`/term_info?offset=${wordObj.id}`);
  ...
  const handleSaveDefinition = async () => {
    ...
    if (!definition) { showNotification("Vui lòng nhập nghĩa cho từ.", "error"); return; }

    try {
      if (USE_MOCK_API) {
        await apiCall("/update_term", { method: "PUT", body: JSON.stringify({ term: word, definition: definition }) });
      } else {
        await apiCall("/update_term", { method: "PUT", body: JSON.stringify({ term: word, definition: definition }) });
      }
      ...
      setWords((prev) => {
        ...
        if (idx !== -1) {
          const arr = [...prev]; arr[idx] = { ...arr[idx], definition, source: "backend" }; return arr;
        }
        // new
        const newItem = { id: prev.length > 0 ? prev[prev.length - 1].id + 1 : 1, word, definition, source: "backend", createdAt: new Date().toISOString() };
        return [...prev, newItem];
      });
    ...
    <div className="vocab-container">
        ...
        <div className="left-panel">
          ...
          <section className="section search-section-fixed">
            ...
            <div className="search-box">
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      // nếu đang hiển thị suggestions và có chọn, ưu tiên chọn đầu tiên
                ...
                {showSuggestions && suggestions && suggestions.length > 0 && (
                  <div style={{
                    ...
                    {suggestions.map((s) => (
                      <div key={s.id} onMouseDown={(e) => { e.preventDefault(); handleSelectSuggestion(s); }} style={{ padding: 10, cursor: "pointer", borderBottom: "1px solid #f1f1f1" }}>
                        <div style={{ fontWeight: "600" }}>{s.word}</div>
                        <div style={{ fontSize: 12, color: "#666" }}>{s.definition || (s.context && s.context[0]) || ""}</div>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="search-filters">
              ...
              <label><input type="checkbox" id="includeDefinitions" checked={includeDefinitions} onChange={(e) => setIncludeDefinitions(e.target.checked)} /> Include definitions</label>
            </div>
          </section>
              ...
              <div className="search-results-container" id="searchResults">
                ...
                {searchResults.map((item) => (
                  <div key={item.id || item.word} className="search-result-item">
                    ...
                    <p style={{ margin: 0, color: "#555", lineHeight: 1.5 }}>{item.definition || "Chưa có nghĩa"}</p>
          ...
          <div className="scrollable-container">
            <section className="vocabulary-display scrollable-content">
              ...
              <div className="word-list" id="wordList">
                ...
                {words.map((item) => {
                  const hasDefinition = item.definition && item.definition.trim() !== "";
                  return (
                    <div key={item.id || item.word} className="word-item">
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#7f8c8d" }}>
                          {hasDefinition ? (<span className="status-badge status-has-definition">✓ Đã có nghĩa</span>) : (<span className="status-badge status-no-definition">✗ Chưa có nghĩa</span>)}
                          {" • Thêm: "}{item.createdAt ? new Date(item.createdAt).toLocaleDateString("vi-VN") : "N/A"}
                        </div>
                        {hasDefinition && (<div className="definition-preview"><strong>Nghĩa:</strong> {item.definition}</div>)}
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => openModalForWord(item)} className="btn-secondary">✏️ {hasDefinition ? "Edit" : "Edit"}</button>
      ...
      {detailOpen && currentWord && (
        <div id="detailModal" className="modal modal-show">
          <div className="modal-content">
            ...
            <div style={{ marginBottom: 10 }}><strong>Definition:</strong> {currentWord.definition || '—'}</div>
      ...
      {modalOpen && (
        <div id="editModal" className="modal modal-show">
          <div className="modal-content">
            <h4>Add/Edit word definition</h4>
            <input type="text" id="editWord" readOnly className="form-input" value={currentWord?.word || ""} />
            <textarea id="editDefinition" placeholder={editDefinition ? `Edit definition of word "${currentWord?.word}"...` : `Add definition for word "${currentWord?.word}"...`} className="form-textarea" value={editDefinition} onChange={(e) => setEditDefinition(e.target.value)} />
            <div className="modal-buttons">
              <button id="cancelEdit" className="btn-secondary" onClick={closeModal}>Hủy</button>
              <button id="saveDefinition" className="btn-primary" onClick={handleSaveDefinition}>Lưu</button>
            </div>
          </div>
        </div>
      )}
